...
When intersection line segment lies on an edge (Fig. \ref{fig:twin}). we call it as \emph{edge intersections}. When there are edge intersections, the intersection line segment will be inserted several times, requiring extra effort to eliminate repetitions. Also, as edge intersections lies on the edge, their intersection contexts are edges, not faces. Finally, in some configurations, edge intersections will cause non-manifold surface as isolated intersection point does, which have to be detected and carefully handled.

There are generally two kinds of edge intersections. If $\gamma(f_1)$ lies on the edge of $f_1$ or $f_2$, but on the inside the face of the other one, then we called it as edge-face intersections (Fig. \ref{fig:twin}(a), (b), (c)). If $l$ lies on the edges of both $f_1$ and $f_2$, we call it as edge-edge intersections (Fig. \ref{fig:twin}(e), (f), (g)). To eliminate repetitive intersection records, when there is edge-face intersection, we treat it as the intersection of three faces $f_1$, $f_2$ and $f_1^\star$ (or $f_2^\star$). When there is edge-edge intersection, we treat it as the intersection of four faces $f_1$, $f_2$, $f_1^\star$ and $f_2^\star$. The $f_1^\star$ (or $f_2^\star$), known as \emph{companion face},  is the neighboring face which shares the intersection edge with $f_1$ (or $f_2$).
