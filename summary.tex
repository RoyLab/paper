...
\caption{In a 2D view, face $\bm{s}$ is on the surface of mesh $P$. However, because we use barycenter $v_c(\bm{s})$ to compute the indicator  and the coordinates of $v_c(\bm{s})$ contain round-off error, we might falsely classify $\bm{s}$ as outside of $P$.}
