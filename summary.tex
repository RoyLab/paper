...
\begin{wrapfigure}{r}[0in]{0in}
\includegraphics[width=1.4in]{p-reps}
\end{wrapfigure}

Using P-rep, each face $f$ with $n$ edges is represented by a supporting plane $\bm{p}_{f,s}$, where the face lies, and a list bounding planes $\{\bm{p}_{f,b}^i \ \vert\  i = 0, 1,...,n-1\}$. Each edge line $\bm{e}_f^i$ is represented by intersection $\bm{p}_{f,s} \cap \bm{p}_{f,b}^i$. Corner vertex $\bm{v}_f^i$ is represented by intersection $\bm{p}_{f,s} \cap \bm{p}_{f,b}^i \cap \bm{p}_{f,b}^{{(i+1)}\bmod{n}}$.
