...
\State
\Function{propagate}{ $\bm{s}$ , $\boldsymbol{\Lambda}(s)$}
    ...
    \For {each neighboring face $\bm{s}_{s, i}$}
        ...
        \Else
            \State \Call{propagate} { $\bm{s}_{s, i}$ , $\boldsymbol{\Lambda}(s)$}
        \EndIf
    \EndFor
\EndFunction
\end{algorithmic}
...
%For example, given a face $\bm{s}$ and one of its vertex $v^i(s)$, if $\lambda_k(v^i(s))=on$, $\lambda_k(s)$ can be any of $on$, $in$ or $out$ (see {\color{red}{Fig. ?}})
